...
async def mock_producer(mocker: MockFixture):
    ...
    producer = AioKafkaProducer("test", UseProducersConfig(producer_ids=["p1", "p2"]))
    await producer.init()
    yield producer
...
@pytest.mark.asyncio
async def test_produce_succeed(mock_producer: AioKafkaProducer):
...
@pytest.mark.asyncio
async def test_produce_retry(mock_producer: AioKafkaProducer):
